...
- **One-off admin commands** run *through* the secret wrapper, e.g.
  `kubectl exec <kwim-service-pod> -c kwim-service -- /app/with-secrets.sh python -m app.<cmd>`.
...
file -> shipped `service/app/kwim.defaults.yaml`**. Two ways to change one in production:
...
Every proposal the gate auto-commits or routes to review posts to `#kwim-review`:
...
- **`python -m app.forget`** - the operator batch/one-off form of the same hard-removal the
...
- **`python -m app.reject_pending`** - bulk-resolve stale unresolved review proposals as
...
`python -m app.rebuild` replays `commit_log` to reconstruct a team's `kwim_<team>` graph.
