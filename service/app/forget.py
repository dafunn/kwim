...
Run (operator, privileged DB creds via the usual KWIM_* env / with-secrets.sh):
    python -m app.forget --team <team> --ids <id1,id2,...> [--commit]
    python -m app.forget --team <team> --select --fact-type code_hub \
...
def _print_plan(team: str, plan: list[dict]) -> None:
    ...
    for p in plan:
        ...
        print(f"    FalkorDB: DETACH DELETE node + orphaned Evidence")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="app.forget", description="KWIM forget (DESTRUCTIVE hard-removal)")
