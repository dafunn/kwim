...
`forget.py` covers governed objects (facts/rules); this covers the other half:
  ...
  - `falkor.forget_node` hardcodes label = "Fact" | "Rule", so `app.forget`
...
Run (operator, privileged creds via the usual KWIM_* env / with-secrets.sh):
    python -m app.forget_semantic --team <team> --ids <id1,id2,...> [--commit]
"""
...
async def _amain(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Confirmation - destructive + irreversible. Same two paths as app.forget:
