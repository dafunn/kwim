...
review = APIRouter(prefix="/v1/review", tags=["review"])


def _require_review_key(team: TeamContext) -> None:
    """Capability gate - mirrors KWIM_PROMOTE_KEYS (main.py wisdom_promote).
...
@review.get("/pending", response_model=list[PendingProposal])
async def review_pending(limit: int = 50, team: TeamContext = CurrentTeam):
    ...
    """
    from .main import State  # local import: avoids a main<->review import cycle
...
@review.post("/{proposal_id}/approve")
...
@review.post("/{proposal_id}/reject")
...
@review.post("/committed/{object_id}/retract")
